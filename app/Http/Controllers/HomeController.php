...
//                    return redirect(route('content-pending', $SocialPostGroupsNew->id));
    ...
    {
        $buffer = BufferPosting::orderBy('id', 'DESC')->paginate(15);
        $group = BufferPosting::join('social_post_groups', 'buffer_postings.group_id', 'social_post_groups.id')->groupBy('buffer_postings.group_id')->select(DB::raw('MAX(social_post_groups.name) as group_name, MAX(social_post_groups.id) as group_id'))->get();
        
        return view('buffer_posting', compact('buffer', 'group'));
    }
...
$buffer = DB::table('buffer_postings')
                ->when($search, function ($query, $search) {
                    return $query->where('post_text', $search);
                })
                ->when($date, function ($query1, $date) {
                    return $query1->where('sent_at', $date);
                })
                ->when($group_type, function ($query2, $group_type) {
                    return $query2->where('group_id', $group_type);
                })
                ->get();
                dd($buffer);
        return view('buffer_posting', compact('buffer', 'group'));
